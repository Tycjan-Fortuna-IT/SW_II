...
namespace SW {
	...
	static void LoadDirectoryEntries();
	...
	static std::filesystem::path m_CurrentDirectory = "C:\\Users\\tycja\\Desktop\\SW_II\\Sandbox\\assets";
	static std::filesystem::path m_AssetsDirectory = "C:\\Users\\tycja\\Desktop\\SW_II\\Sandbox\\assets";
	static std::vector<File> m_DirectoryEntries = {};	/** @brief The directory entries. */
	...
	static void DirectoryEntryPopup(const std::filesystem::path& entry) {
	...
	static void DirectoryTreeViewRecursive(const std::filesystem::path& path)
	{
		...
		for (const std::filesystem::directory_entry& entry : std::filesystem::directory_iterator(path)) {
			...
			if (!entryIsFile) {
				if (opened) {
					DirectoryTreeViewRecursive(entryPath);
	...
	static void RenderSideView()
	{
		...
		if (ImGui::BeginTable("SideViewTable", 1, flags, ImGui::GetContentRegionAvail())) {
			...
			if (opened) {
				DirectoryTreeViewRecursive(m_AssetsDirectory);
	...
	static void RenderBody()
	{
		...
		if (ImGui::BeginTable("BodyTable", columnCount, flags)) {
			...
			for (const File& entry : m_DirectoryEntries) {
				...
				DirectoryEntryPopup(entry.FilePath);
