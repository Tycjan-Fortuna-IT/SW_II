...
namespace SW {
	...
	public:
		 ...
		 * @param tag The tag to display before formatted message.
		 * @param format The message to log. Must be a string literal.
		...
		static void PrintMessage(LogType type, LogLevel level, std::string_view tag, std::string_view format = "", Args&&... args)
		{
			...
			switch (level) {
				case LOG_LEVEL_TRACE:
					logger->trace("{0}{1}", tag, message); break;
				case LOG_LEVEL_INFO:
					logger->info("{0}{1}", tag, message); break;
				case LOG_LEVEL_WARN:
					logger->warn("{0}{1}", tag, message); break;
				case LOG_LEVEL_DEBUG:
					logger->debug("{0}{1}", tag, message); break;
				case LOG_LEVEL_ERROR:
					logger->error("{0}{1}", tag, message); break;
	 ...
	 */
	#define SW_LOG(type, level, msg, ...) ::SW::LogSystem::PrintMessage(::SW::LogType::type, ::SW::LogLevel::level, "", msg __VA_OPT__(, ) ##__VA_ARGS__)
