...
namespace SW
{
	...
	void ScriptStorage::ShutdownEntityStorage(u64 scriptID, u64 entityID)
	{
		...
		for (auto& [fieldID, fieldStorage] : EntityStorage[entityID].Fields) fieldStorage.m_ValueBuffer.Release();
	...
	void ScriptStorage::CopyTo(ScriptStorage& other) const
	{
		...
		for (const auto& [entityID, entityStorage] : EntityStorage)
		{
			...
			for (const auto& [fieldID, fieldStorage] : entityStorage.Fields)
			{
				...
				{
					ASSERT(false,
					       "Cannot copy script data for field {}. The field is no longer contained in the script.",
	...
	void ScriptStorage::CopyEntityStorage(u64 entityID, u64 targetEntityID, ScriptStorage& targetStorage) const
	{
		...
		{
			ASSERT(false,
			       "Cannot copy script storage to entity {} because InitializeScriptStorage hasn't been called for it.",
			       targetEntityID);
			return;
		}
		...
		{
			ASSERT(false, "Cannot copy script storage from entity {} to entity {} because they have different scritps!",
	...
	void ScriptStorage::Clear()
	{
		for (auto& [entityID, entityStorage] : EntityStorage)
		{
			for (auto& [fieldID, fieldStorage] : entityStorage.Fields) fieldStorage.m_ValueBuffer.Release();
		}

		EntityStorage.clear();
	}

	void ScriptStorage::InitializeFieldStorage(EntityScriptStorage& storage, u32 fieldID,
