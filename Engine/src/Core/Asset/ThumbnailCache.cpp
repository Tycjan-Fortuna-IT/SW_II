...
namespace SW {
	...
	Texture2D** ThumbnailCache::GetThumbnail(const std::filesystem::path& itemPath, AssetHandle handle, Timestamp lastModified)
	{
		...
		*/
		...
		for (const std::filesystem::directory_entry& entry : std::filesystem::directory_iterator(cachePath)) {
			...
			u64 itemHandle = std::stoull(handleStr);
			u64 modTime = std::stoull(modTimeStr);
