...
namespace SW
{
	...
	struct Vector4 final
	{
		 ...
		 * - Two, three or four elements: The components will be set to these values, in order with the last value
		...
		Vector4(std::initializer_list<T> values)
		{
			...
			{
				SYSTEM_WARN(
				    "Too many arguments passed for the Vector3D constructor! Expected 0, 1, 2, 3 or 4, received: {}",
				    values.size());
			}
		 ...
		 * @note If the other vector's x, y, z or w component is zero, an error message is logged and the division is
		 * not performed.
		 */
		Vector4<T> operator/(const Vector4<T>& other) const
		{
			if (other.x == static_cast<T>(0) || other.y == static_cast<T>(0) || other.z == static_cast<T>(0) ||
			    other.w == static_cast<T>(0))
			{
