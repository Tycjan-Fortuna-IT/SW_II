...
namespace SW::GUI
{
	...
	namespace Layout
	{
		...
		bool BeginMenuBar(const ImRect& barRectangle)
		{
			...
			ImRect clip(IM_ROUND(ImMax(window->Pos.x, bar.Min.x + window->WindowBorderSize + window->Pos.x - 10.0f)),
			            IM_ROUND(bar.Min.y + window->WindowBorderSize + window->Pos.y),
			            IM_ROUND(ImMax(bar.Min.x + window->Pos.x,
			                           bar.Max.x - ImMax(window->WindowRounding, window->WindowBorderSize))),
			...
			window->DC.CursorPos = window->DC.CursorMaxPos =
			    ImVec2(bar.Min.x + window->Pos.x, bar.Min.y + window->Pos.y);
		...
		{
			return ImGui::TreeNodeEx(name, flags | ImGuiTreeNodeFlags_AllowItemOverlap | ImGuiTreeNodeFlags_Framed,
			                         "%s", name);
		}
	...
	namespace Components
	{
		...
		void ItemActivityOutline(OutlineFlags flags /*= OutlineFlags_All*/,
		                         ImColor activeCol /*= GUI::Theme::ActivityOutline*/,
		                         ImColor hoverCol /*= GUI::Theme::Outline*/,
		...
		bool Checkbox(bool* value, bool center)
		{
			...
			Components::ItemActivityOutline(OutlineFlags_All, GUI::Theme::ActivityOutline, GUI::Theme::ActivityOutline,
			                                2.f);
		...
		bool Toggle(bool* value, bool center)
		{
			...
			ImU32 col_bg = ImGui::GetColorU32(*value ? ImGui::ColorConvertU32ToFloat4(GUI::Theme::Selection)
	...
	namespace Widgets
	{
		              ...
		              f32 splitterLongAxisSize /*= -1.0f*/)
		{
			...
			return ImGui::SplitterBehavior(bb, id, splitVertically ? ImGuiAxis_X : ImGuiAxis_Y, size1, size2, minSize1,
			                               minSize2, 0.0f);
		}
		                 ...
		                 const ImRect* cliprect, f32 wrapWidth)
		{
			...
			const ImVec2 textSize =
			    textSizeIfKnown ? *textSizeIfKnown : ImGui::CalcTextSize(text, textDisplayEnd, false, wrapWidth);

			const ImVec2* clipMin = cliprect ? &cliprect->Min : &posMin;
			const ImVec2* clipMax = cliprect ? &cliprect->Max : &posMax;

			// Align whole block. We should defer that to the better rendering function when we'll have support for
			...
			drawList->AddText(nullptr, 0.0f, pos, ImGui::GetColorU32(ImGuiCol_Text), text, textDisplayEnd, wrapWidth,
			                  &fineClipRect);
		}
		                 ...
		                 const ImVec2* textSizeIfKnown, const ImVec2& align, const ImRect* clipRect, f32 wrapWidth)
		{
			...
			ImGuiContext& g     = *GImGui;
			ImGuiWindow* window = g.CurrentWindow;
			ClippedText(window->DrawList, posMin, posMax, text, textDisplayEnd, textSizeIfKnown, align, clipRect,
			            wrapWidth);
			if (g.LogEnabled)
...

			{
				...
				if (ImGui::Button("##folder_picker", {isPicked ? w - bw : w, ImGui::GetFrameHeight()}))
				{
					std::filesystem::path pickedPath =
					    FileSystem::OpenFolderDialog(isRelative ? "" : relative.string().c_str());
		...
		bool Vector2Table(std::vector<glm::vec2>* vector, f32 defaultValue /*= 0.f*/)
		{
			...
			GUI::Components::ScalarInput<u64>(&pointsCount, 1u, 1u);
			...
			if (ImGui::BeginTable("##table_vector2", 2, flags))
			{
				...
				for (int i = 0; i < (int)vector->size(); i++)
				{
					...
					GUI::Components::Vector2Input(&point, defaultValue);
		...
		bool Vector3Table(std::vector<glm::vec3>* vector, f32 defaultValue /*= 0.f*/)
		{
			...
			if (ImGui::BeginTable("##table_vector3", 2, flags))
			{
				...
				for (int i = 0; i < (int)vector->size(); i++)
				{
					...
					ImGui::PushID(vectorId.c_str());
					GUI::Components::Vector3Input(&point, defaultValue);
	...
	namespace Properties
	{
		...
		)
		{
			...
			GUI::Properties::BeginPropertyGrid(label, tooltip);
			...
			// this nasty way, because of variations and flickering of ImGui::GetContentRegionAvail() at certain column
			// sizes.
