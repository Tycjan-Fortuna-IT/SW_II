...
namespace SW
{
	...
	void SceneViewportPanel::OnUpdate(Timestep dt)
	{
		...
		glm::vec3 dampedPosition =
		    Math::SmoothDamp(position, finalPosition, m_TranslationVelocity, m_TranslationDampening, 10000.0f, dt);
		glm::vec2 dampedYawPitch =
		    Math::SmoothDamp(yawPitch, finalYawPitch, m_RotationVelocity, m_RotationDampening, 1000.0f, dt);
		...
		if (IsSceneLoaded())
		{
			...
			if (EditorSettings::Get().ShowPhysicsColliders)
			{
				...
				// Debug Camera Visualization
				{
					...
					for (auto&& [handle, cc] : m_ActiveScene->GetRegistry().GetEntitiesWith<CameraComponent>().each())
					{
						...
						if (cc.Primary)
						{
							...
							const glm::mat4 inv           = glm::inverse(mainCamera.GetProjectionMatrix() *
							                                             glm::inverse(entity.GetWorldSpaceTransformMatrix()));
				...
				{
					for (auto&& [handle, bcc] :
					     m_ActiveScene->GetRegistry().GetEntitiesWith<BoxCollider2DComponent>().each())
					{
						Entity entity = {handle, m_ActiveScene};
				...
				{
					for (auto&& [handle, ccc] :
					     m_ActiveScene->GetRegistry().GetEntitiesWith<CircleCollider2DComponent>().each())
					{
						Entity entity = {handle, m_ActiveScene};
				...
				{
					for (auto&& [handle, pcc] :
					     m_ActiveScene->GetRegistry().GetEntitiesWith<PolygonCollider2DComponent>().each())
					{
						Entity entity = {handle, m_ActiveScene};
						...
						if (m_ActiveScene->GetCurrentState() == SceneState::Play)
						{
							// TODO: Investigate this visualization offset difference between play/edit state
						...
						for (u64 i = 0; i < pcc.Vertices.size(); ++i)
						{
							...
							const glm::vec4 p1 =
							    transform * glm::vec4(pcc.Vertices[(i + 1) % pcc.Vertices.size()], 0.0f, 1.0f);
				...
				{
					for (auto&& [handle, djc] :
					     m_ActiveScene->GetRegistry().GetEntitiesWith<DistanceJoint2DComponent>().each())
					{
						...
						const glm::mat4 originTransformMatrix =
						    glm::translate(glm::mat4(1.0f), originTransform.Position) *
						...
						const glm::mat4 connectedTransformMatrix =
						    glm::translate(glm::mat4(1.0f), connectedTransform.Position) *
				...
				{
					for (auto&& [handle, rjc] :
					     m_ActiveScene->GetRegistry().GetEntitiesWith<RevolutionJoint2DComponent>().each())
					{
						...
						const glm::mat4 originCircleTransformMatrix =
						    glm::translate(glm::mat4(1.0f), originTransform.Position) *
						    glm::rotate(glm::mat4(1.0f), originTransform.Rotation.z, glm::vec3(0.0f, 0.0f, 1.0f)) *
						    glm::translate(glm::mat4(1.0f), glm::vec3(rjc.OriginAnchor, 0.001f)) *
				...
				{
					for (auto&& [handle, pjc] :
					     m_ActiveScene->GetRegistry().GetEntitiesWith<PrismaticJoint2DComponent>().each())
					{
						...
						glm::vec3 moving =
						    originTransform.Position + glm::vec3(pjc.OriginAnchor.x, pjc.OriginAnchor.y, 0.001f);
						...
						glm::vec3 lowerPoint =
						    moving + glm::vec3(lower * glm::cos(angle), lower * glm::sin(angle), 0.001f);
						glm::vec3 upperPoint =
						    moving + glm::vec3(upper * glm::cos(angle), upper * glm::sin(angle), 0.001f);
				...
				{
					for (auto&& [handle, sjc] :
					     m_ActiveScene->GetRegistry().GetEntitiesWith<SpringJoint2DComponent>().each())
					{
						Entity originEntity = {handle, m_ActiveScene};
				...
				{
					for (auto&& [handle, wjc] :
					     m_ActiveScene->GetRegistry().GetEntitiesWith<WheelJoint2DComponent>().each())
					{
				...
				{
					for (auto&& [handle, asc] :
					     m_ActiveScene->GetRegistry().GetEntitiesWith<AudioSourceComponent>().each())
					{
						if (!asc.Is3D)
							continue;
						...
						Renderer2D::DrawCircle(glm::rotate(transform, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f)),
						                       glm::vec4(1.f, 0.5f, 0.f, 1.f), 0.02f);
						Renderer2D::DrawCircle(glm::rotate(transform, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f)),
						                       glm::vec4(1.f, 0.5f, 0.f, 1.f), 0.02f);
	...
	void SceneViewportPanel::OnRender()
	{
		...
		if (OnBegin(ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse))
		{
			...
			if (!isSceneLoaded || m_ActiveScene->GetCurrentState() == SceneState::Edit)
			{
				if (ImGui::BeginDragDropTarget())
				{
					if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("Scene"))
					{
						...
						Scene* newScene =
						    SceneSerializer::Deserialize(ProjectContext::Get()->GetAssetDirectory() / metadata.Path);
			...
			ImGui::SetNextItemAllowOverlap();
			{
				...
				ImVec2 resolutionTextPosition = {ImGui::GetCursorPosX() + m_ViewportSize.x - resolutionTextSize.x -
				                                     framePadding.x,
				                                 startCursorPos.y + framePadding.y};
			...
			if (SelectionManager::IsSelected() && isSceneLoaded && m_ActiveScene->GetCurrentState() != SceneState::Play)
			{
				...
				if (ImGuizmo::IsUsing())
				{
					...
					const glm::mat4& parentWorldTransform =
					    parent ? parent.GetWorldSpaceTransformMatrix() : glm::mat4(1.0f);

					glm::vec3 translation, rotation, scale;
					Math::DecomposeTransform(glm::inverse(parentWorldTransform) * transform, translation, rotation,
					                         scale);
	...
	bool SceneViewportPanel::OnMouseButtonPressed(MouseCode code)
	{
		...
		{
			if (ImGuizmo::IsUsing() || (ImGuizmo::IsOver() && SelectionManager::IsSelected()) || m_IsGizmoBarHovered ||
			    m_IsToolbarHovered)
				return false;
	...
	void SceneViewportPanel::RenderSceneToolbar(ImVec2 startCursorPos)
	{
		...
		const ImVec2 gizmoPosition = {m_ViewportBoundsMin.x + m_ViewportSize.x / 2.f + m_ToolbarPosition.x,
		                              m_ViewportBoundsMin.y + m_ToolbarPosition.y};
		const ImRect bb(gizmoPosition.x - ((buttonSize.x + 1.f) * (buttonCount / 2.f + 0.5f)), gizmoPosition.y + 16.f,
		...
		ImGui::SetCursorPos(
		    {m_ViewportSize.x / 2.f + m_ToolbarPosition.x - (buttonSize.x * (buttonCount / 2.f + 0.75f)),
		     startCursorPos.y + tempGizmoPosition.y + 20.f});
		ImGui::BeginGroup();
		{
			...
			if (GUI::Components::ToggleButton(&isStopped, SW_ICON_STOP, SW_ICON_STOP, false))
			{
				if (currentState != SceneState::Edit)
				{
					...
					if (SelectionManager::IsSelected()) // If entity was created during run time and was selected within
					                                    // the editor
	...
	void SceneViewportPanel::RenderGizmoToolbar(ImVec2 startCursorPos)
	{
		...
		const ImVec2 gizmoPosition = {m_ViewportBoundsMin.x + m_GizmoToolbarPosition.x,
		                              m_ViewportBoundsMin.y + m_GizmoToolbarPosition.y};
		const ImRect bb(gizmoPosition.x, gizmoPosition.y, gizmoPosition.x + buttonSize.x + 8,
		                gizmoPosition.y + (buttonSize.y + 2) * (buttonCount + 0.5f));
		...
		ImGui::SetCursorPos(
		    {startCursorPos.x + tempGizmoPosition.x + framePadding.x, startCursorPos.y + tempGizmoPosition.y});
		ImGui::BeginGroup();
		{
			ImGui::PushStyleVar(ImGuiStyleVar_FrameBorderSize, 0.0f);
			ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, {1, 1});
