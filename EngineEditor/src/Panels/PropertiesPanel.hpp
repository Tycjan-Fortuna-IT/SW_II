...
{
	/**
	 * @brief The PropertiesPanel class represents a panel that displays for the selected object its properties and
	 * components. Allows to edit the properties and add/remove components.
	 */
	...
	public:
		...
		void DrawComponent(Entity entity, Fn fn, bool removable = true)
		{
			...
			const bool open =
			    ImGui::TreeNodeEx(reinterpret_cast<void*>(id), treeFlags, "%s", m_ComponentNames[id].c_str());
			...
			if (ImGui::BeginPopup("ComponentSettings"))
			{
				...
				{
					m_SceneViewportPanel->GetCurrentScene()->CopyComponentIntoScene<T>(m_ComponentCopyScene, entity,
					                                                                   m_ComponentCopyEntity);
				}

				if (ImGui::MenuItemEx("Paste", SW_ICON_CONTENT_PASTE))
				{
					m_ComponentCopyScene->CopyComponentIntoScene<T>(m_SceneViewportPanel->GetCurrentScene(),
					                                                m_ComponentCopyEntity, entity);
				}
