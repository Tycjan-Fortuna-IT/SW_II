...
namespace SW
{
	...
	{
		GUI::ScopedColor PanelBackground(ImGuiCol_WindowBg,
		                                 GUI::Colors::Darken(GUI::Appearance::GetColors().ChildBackground, 0.02f));
		...
		if (OnBegin())
		{
			...
			if (m_SelectedItem &&
			    GUI::Popups::DrawAddNewFilePopup(m_AssetsDirectory / m_SelectedItem->Path, &m_OpenNewFileModal))
				LoadDirectoryEntries();
	...
	void AssetPanel::LoadDirectoryEntries()
	{
		...
		AssetSourceItem* newSelectedItem =
		    m_AssetTree->FindChildItemByPath(m_AssetTree->GetRootItem(), oldSelectedPath);
			...
			for (auto it = paths.rbegin(); it != paths.rend(); ++it)
			{
				...
				if (ImGui::Button(filename.c_str(), {ImGui::CalcTextSize(filename.c_str()).x * 1.5f, 34.f}))
				{
					const std::filesystem::path path =
					    std::filesystem::relative(*it, ProjectContext::Get()->GetAssetDirectory());
	...
	{
		constexpr ImGuiTableFlags flags = ImGuiTableFlags_NoPadInnerX | ImGuiTableFlags_NoPadOuterX |
	...
	void AssetPanel::DrawBody()
	{
		...
		constexpr ImGuiTableFlags flags = ImGuiTableFlags_ContextMenuInBody | ImGuiTableFlags_ScrollY |
		                                  ImGuiTableFlags_PadOuterX | ImGuiTableFlags_BordersOuterV |
		...
		if (ImGui::BeginTable("BodyTable", columnCount, flags))
		{
			for (int i = 0; i < (int)m_SelectedItem->Children.size(); i++)
			{
				...
				const ImVec2 imageSize = {backgroundThumbnailSize.x - padding * 2.0f - 3.f,
				                          backgroundThumbnailSize.y - padding * 2.0f};
				...
				const ImVec4 borderColor     = m_IsTableHovered && ImGui::IsMouseHoveringRect(min, max)
				                                   ? ImGui::ColorConvertU32ToFloat4(GUI::Theme::Selection)
				                                   : ImGui::ColorConvertU32ToFloat4(GUI::Theme::PropertyField);
				...
				if (!item->Thumbnail)
				{
					...
					// ---- !IMPORTANT! ----
					/*
					    Actually and edge case, when asset was just created e.g. importing a FontSource and creating
					   with it a Font, it does not have the correct modification time, since it happened instantly as
					   for the disk save takes some time and updates the modification time in a deffered way.
					*/
					const u64 modificationTime =
					    FileSystem::GetLastWriteTime(ProjectContext::Get()->GetAssetDirectory() / item->Path);
					...
					if (item->Type == AssetType::Sprite)
					{
						...
						thumbnail.Width =
						    std::roundf(abs((*spriteAsset)->TexCordUpRight.x - (*spriteAsset)->TexCordLeftDown.x) *
						                (f32)texture->GetWidth());
						thumbnail.Height =
						    std::roundf(abs((*spriteAsset)->TexCordUpRight.y - (*spriteAsset)->TexCordLeftDown.y) *
						                (f32)texture->GetHeight());
				...
				if (item->Type == AssetType::Animation2D)
				{
					...
					if (framesCount)
					{
						...
						item->Thumbnail.Width =
						    std::roundf(abs((*sprite)->TexCordUpRight.x - (*sprite)->TexCordLeftDown.x) *
						                (f32)(*texture)->GetWidth());
						item->Thumbnail.Height =
						    std::roundf(abs((*sprite)->TexCordUpRight.y - (*sprite)->TexCordLeftDown.y) *
						                (f32)(*texture)->GetHeight());
				...
				ImGui::Image(whiteTexId, typeColorFrameSize, {0, 0}, {1, 1},
				             ImGui::ColorConvertU32ToFloat4(item->Color));

				const ImVec2 rectMin  = ImGui::GetItemRectMin();
				const ImVec2 rectSize = ImGui::GetItemRectSize();
				const ImRect clipRect = ImRect(
				    {rectMin.x + padding * 1.0f, rectMin.y + padding * ((f32)m_ThumbnailSize / 100.f)},
				    {rectMin.x + rectSize.x, rectMin.y + scaledThumbnailSizeX -
				                                 GUI::Appearance::GetFonts().SmallFont->FontSize - padding * 4.0f});
				GUI::Widgets::ClippedText(clipRect.Min, clipRect.Max, filename.c_str(), filenameEnd, nullptr, {0, 0},
				                          nullptr, clipRect.GetSize().x);

				ImGui::SetCursorPos({cursorPos.x + padding * 2.0f,
				                     cursorPos.y + backgroundThumbnailSize.y -
				                         GUI::Appearance::GetFonts().DefaultBoldFont->FontSize - padding * 2.0f});
				ImGui::BeginDisabled();
			...
			if (ImGui::BeginDragDropTarget())
			{
				if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("Entity"))
				{
					...
					std::filesystem::path newFilePath =
					    FileSystem::GetUniqueFilename(ProjectContext::Get()->GetAssetDirectory() /
					                                  m_SelectedItem->Path / (entity.GetTag() + ".sw_prefab"));

					AssetManager::CreateNew<Prefab>(
					    std::filesystem::relative(newFilePath, ProjectContext::Get()->GetAssetDirectory()), entity);
