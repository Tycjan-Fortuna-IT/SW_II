...
namespace SW
{
	...
	void PropertiesPanel::OnRender()
	{
		...
		if (OnBegin())
		{
			...
			Entity entity =
			    m_SceneViewportPanel->GetCurrentScene()->TryGetEntityByID(SelectionManager::GetSelectionID());
			...
			DrawComponent<SpriteComponent>(
			    ...
			    [](SpriteComponent& component) {
				    ...
				    if (GUI::Properties::AssetSearchProperty<Sprite>(&handle, "Sprite",
				                                                     "Sprite to be used (transparency is supported)"))
				    {
					    component.Handle = handle;
				    }
				    GUI::Properties::ScalarInputProperty<f32>(
				        &component.TilingFactor, "Tiling",
				        "Tiling factor of the texture (how many times the texture should be repeated)", 1.f, 5.f, 0.f,
				        20.f);
				    GUI::Properties::ScalarInputProperty<int>(
				        &component.ZIndex, "Z-Index", "Z-Index of the sprite (higher z-index will be rendered on top)");
				    GUI::Properties::EndProperties();
			    },
			    true);
			...
			DrawComponent<AnimatedSpriteComponent>(
			    entity,
			    [](AnimatedSpriteComponent& component) {
				    GUI::Properties::BeginProperties("##sprite_property");

				    AssetHandle handle = component.DefaultAnimation ? (*component.DefaultAnimation)->GetHandle() : 0u;
				    if (GUI::Properties::AssetSearchProperty<Animation2D>(
				            &handle, "Default",
				            "Default animation to be used if nothing is being played (if not specified - nothing will "
				            "be visible)"))
				    {
				    ...
				    GUI::Properties::AssetDropdownTableMapProperty<std::string, Animation2D>(&component.Animations,
				                                                                             "Animations");
			...
			DrawComponent<CircleComponent>(
			    ...
			    [](CircleComponent& component) {
				    ...
				    GUI::Properties::ScalarSliderProperty<f32>(
				        &component.Thickness, "Thickness", "Decreasing thickness will start to create a ring inside",
				        0.f, 1.f);
				    GUI::Properties::ScalarSliderProperty<f32>(
				        &component.Fade, "Fade", "How much the circle starts to fade from outside", 0.f, 4.f);
				    GUI::Properties::EndProperties();
			    },
			    true);
			...
			DrawComponent<TextComponent>(
			    ...
			    [](TextComponent& component) {
				    ...
				    if (component.Handle)
				    {
					    GUI::Properties::MultiLineTextInputProperty<500>(&component.TextString, "Text",
					                                                     "Text to display (max 500 characters)");
					    GUI::Properties::Vector4ColorPickerProperty(&component.Color, "Color", "Color of the text");
					    GUI::Properties::ScalarDragProperty<f32>(&component.Kerning, "Kerning",
					                                             "The space between the characters", 1.f, 0.f);
					    GUI::Properties::ScalarDragProperty<f32>(&component.LineSpacing, "Line Spacing",
					                                             "The space between the lines", 1.f, 0.f);
				    }
				    GUI::Properties::EndProperties();
			    },
			    true);
			...
			DrawComponent<CameraComponent>(
			    ...
			    [](CameraComponent& component) {
				    ...
				    GUI::Properties::CheckboxProperty(
				        &component.Primary, "Primary",
				        "Whether this camera is the primary camera (only one camera can be primary)");

				    ProjectionType type = component.Camera.GetProjectionType();

				    if (GUI::Properties::SelectableProperty<ProjectionType>(
				            &type,
				            {
				    ...
				    if (type == ProjectionType::Orthographic)
				    {
					    ...
					    if (GUI::Properties::ScalarDragProperty<f32>(&orthoSize, "Size", "Size of the camera", 1.f,
					                                                 0.f))
						    component.Camera.SetOrthographicSize(orthoSize);

					    f32 orthoNear = component.Camera.GetOrthographicNearClip();
					    if (GUI::Properties::ScalarDragProperty<f32>(&orthoNear, "Near Clip",
					                                                 "The near clip of the camera", 1.f, 0.f))
						    component.Camera.SetOrthographicNearClip(orthoNear);

					    f32 orthoFar = component.Camera.GetOrthographicFarClip();
					    if (GUI::Properties::ScalarDragProperty<f32>(&orthoFar, "Far Clip",
					                                                 "The far clip of the camera", 1.f, 0.f))
				    ...
				    else if (type == ProjectionType::Perspective)
				    {
					    ...
					    if (GUI::Properties::ScalarSliderProperty<f32>(
					            &verticalFov, "Vertical FOV", "Vertical field of view of the camera in degrees", 0.f,
					            180.f))
					    ...
					    if (GUI::Properties::ScalarDragProperty<f32>(&perspectiveNear, "Near Clip",
					                                                 "The near clip of the camera", 1.f, 0.f))
						    component.Camera.SetPerspectiveNearClip(perspectiveNear);

					    f32 perspectiveFar = component.Camera.GetPerspectiveFarClip();
					    if (GUI::Properties::ScalarDragProperty<f32>(&perspectiveFar, "Far Clip",
					                                                 "The far clip of the camera", 1.f, 0.f))
			...
			DrawComponent<ScriptComponent>(
			    entity,
			    [this, id](ScriptComponent& component) {
				    ...
				    if (component.ScriptID)
				    {
					    ...
					    Coral::Type& reflectionHelper =
					        scriptingCore.GetCoreAssembly()->Assembly->GetType("SW.ReflectionHelper");

					    Coral::String assemblyName =
					        Coral::String::New(scriptingCore.GetAppAssembly()->Assembly->GetName());
					    ...
					    for (auto& [fieldId, fieldStorage] : fieldStorages)
					    {
						    ...
						    else
						    {
							    ...
							    {
							    case DataType::Byte:
								    ASSERT(false, "Byte not yet supported!");
								    break;
							    case DataType::Short:
								    ASSERT(false, "Short not yet supported!");
								    break;
							    case DataType::UShort:
								    ASSERT(false, "UShort not yet supported!");
								    break;
							    case DataType::Int:
								    ASSERT(false, "Int not yet supported!");
								    break;
							    case DataType::UInt:
								    ASSERT(false, "UInt not yet supported!");
								    break;
							    case DataType::Long:
								    ASSERT(false, "Long not yet supported!");
								    break;
							    case DataType::ULong:
								    ASSERT(false, "ULong not yet supported!");
								    break;
							    case DataType::Float: {
								    f32 value = fieldStorage.GetValue<f32>();

								    if (GUI::Properties::ScalarInputProperty<f32>(&value,
								                                                  fieldStorage.GetName().data()))
								    {
							    ...
							    case DataType::Double:
								    ASSERT(false, "Double not yet supported!");
								    break;
							    case DataType::Bool:
								    ASSERT(false, "Bool not yet supported!");
								    break;
							    case DataType::Entity: {
								    if (!m_SceneViewportPanel->GetCurrentScene()->IsPlaying())
								    {
									    ...
									    if (GUI::Properties::EntityDropdownProperty(
									            &value, m_SceneViewportPanel->GetCurrentScene(),
									            std::string(fieldStorage.GetName()).c_str(), "Entity to be used"))
									    {
										    fieldStorage.SetValue(value);
									    }
								    }
							    ...
							    case DataType::Prefab: {
								    if (!m_SceneViewportPanel->GetCurrentScene()->IsPlaying())
								    {
									    ...
									    if (GUI::Properties::AssetSearchProperty<Prefab>(
									            &value, std::string(fieldStorage.GetName()).c_str(),
									            "Prefab to be used"))
									    {
										    fieldStorage.SetValue(value);
									    }
			...
			DrawComponent<RigidBody2DComponent>(
			    ...
			    [](RigidBody2DComponent& component) {
				    ...
				    GUI::Properties::SelectableProperty<PhysicBodyType>(
				        &component.Type,
				        {{"Static", PhysicBodyType::Static},
				         {"Kinematic", PhysicBodyType::Kinematic},
				         {"Dynamic", PhysicBodyType::Dynamic}},
				        "Body Type",
				        "The type of the body, static bodies are immovable, kinematic bodies are movable but not "
				        "affected by forces, dynamic bodies are movable and affected by forces");
				    if (component.Type == PhysicBodyType::Kinematic || component.Type == PhysicBodyType::Dynamic)
				    {
					    GUI::Properties::CheckboxProperty(
					        &component.AutoMass, "Auto mass",
					        "Whether the body's mass should be determined automatically or based on set mass");
					    if (!component.AutoMass)
					    {
						    GUI::Properties::ScalarDragProperty<f32>(&component.Mass, "Mass", "The mass of the body",
						                                             1.f, 0.f);
					    }
				    }
				    if (component.Type == PhysicBodyType::Dynamic)
				    {
					    GUI::Properties::ScalarInputProperty<f32>(&component.GravityScale, "Gravity Scale",
					                                              "The gravity scale of the body", 1, 5);
					    GUI::Properties::ScalarSliderProperty<f32>(
					        &component.Friction, "Friction", "The friction of the body (how much it resists movement)",
					        0.f, 1.f);
					    GUI::Properties::ScalarSliderProperty<f32>(&component.Restitution, "Restitution",
					                                               "The restitution of the body (how much it bounces)",
					                                               0.f, 1.f);
					    GUI::Properties::ScalarSliderProperty<f32>(
					        &component.RestitutionThreshold, "Restitution Threshold",
					        "The velocity threshold for the restitution", 0.f, 1.f);
					    GUI::Properties::ScalarSliderProperty<f32>(
					        &component.LinearDamping, "Linear Damping",
					        "The linear damping of the body (how much it resists movement)", 0.f, 1.f);
					    GUI::Properties::ScalarSliderProperty<f32>(
					        &component.AngularDamping, "Angular Damping",
					        "The angular damping of the body (how much it resists rotation)", 0.f, 1.f);
					    GUI::Properties::CheckboxProperty(
					        &component.FixedRotation, "Fixed Rotation",
					        "Whether the body should not be rotated by the physics system");
					    GUI::Properties::CheckboxProperty(&component.AllowSleep, "Allow Sleep",
					                                      "Whether the body should be allowed to sleep");
					    GUI::Properties::CheckboxProperty(&component.InitiallyAwake, "Initially Awake",
					                                      "Whether the body should be allowed to sleep");
					    GUI::Properties::CheckboxProperty(&component.IsBullet, "Is Bullet",
					                                      "Is this a fast moving body that should be prevented from "
					                                      "tunneling through other moving bodies?");
				    }
				    GUI::Properties::EndProperties();
			    },
			    true);
			...
			DrawComponent<BoxCollider2DComponent>(
			    ...
			    [](BoxCollider2DComponent& component) {
				    ...
				    GUI::Properties::Vector2InputProperty(&component.Offset, "Offset",
				                                          "Offset of the collider from origin");
				    GUI::Properties::ScalarInputProperty<f32>(&component.Density, "Density",
				                                              "Density of the collider (mass = density * area)", 1.f,
				                                              10.f, 0.f);
				    GUI::Properties::CheckboxProperty(&component.IsSensor, "Is Sensor?",
				                                      "Whether to react to the collision or just sense it.");
				    GUI::Properties::EndProperties();
			    },
			    true);
			...
			DrawComponent<CircleCollider2DComponent>(
			    ...
			    [](CircleCollider2DComponent& component) {
				    ...
				    GUI::Properties::ScalarInputProperty<f32>(&component.Radius, "Radius", "Radius of the collider",
				                                              1.f, 5.f, 0.1f);
				    GUI::Properties::Vector2InputProperty(&component.Offset, "Offset",
				                                          "Offset of the collider from origin");
				    GUI::Properties::ScalarInputProperty<f32>(&component.Density, "Density",
			...
			DrawComponent<PolygonCollider2DComponent>(
			    entity,
			    [](PolygonCollider2DComponent& component) {
				    GUI::Properties::BeginProperties("##polygon_collider_2d_property");
				    GUI::Properties::Vector2TableProperty(
				        &component.Vertices, "Edges",
				        "List of all of the edges of the polygon collider (minimum 3 edges!)");
				    GUI::Properties::Vector2InputProperty(&component.Offset, "Offset",
				                                          "Offset of the collider from origin");
				    GUI::Properties::ScalarInputProperty<f32>(&component.Density, "Density",
				                                              "Density of the collider (mass = density * area)", 1.f,
			...
			DrawComponent<BuoyancyEffector2DComponent>(
			    entity,
			    [](BuoyancyEffector2DComponent& component) {
				    GUI::Properties::BeginProperties("##buoancy_effector_2d_property");
				    GUI::Properties::ScalarDragProperty<f32>(
				        &component.DragMultiplier, "Drag Multiplier",
				        "The drag multiplier of the effector (how much it resists movement)");
				    GUI::Properties::ScalarDragProperty<f32>(&component.FlowMagnitude, "Flow Magnitude",
				                                             "The magnitude of the flow force in Newtons");
				    f32 angle = glm::degrees(component.FlowAngle);
				    GUI::Properties::ScalarDragProperty<f32>(&angle, "Flow Angle",
				                                             "The angle of the flow force in degrees");
			...
			DrawComponent<DistanceJoint2DComponent>(
			    entity,
			    [this](DistanceJoint2DComponent& component) {
				    GUI::Properties::BeginProperties("##distance_joint_2d_property");
				    GUI::Properties::EntityDropdownProperty(&component.ConnectedEntityID,
				                                            m_SceneViewportPanel->GetCurrentScene(), "Connected entity",
				                                            "The joint will connect to this entity's rigid body");
				    GUI::Properties::Vector2InputProperty(&component.OriginAnchor, "Origin Anchor",
				                                          "The anchor point of this body");
				    GUI::Properties::Vector2InputProperty(&component.ConnectedAnchor, "Connected Anchor",
				                                          "The anchor point of the connected body");
				    GUI::Properties::ScalarDragProperty<f32>(&component.BreakingForce, "Breaking Force",
				                                             "The force acting on the joint to break it in Newtons",
				                                             1.f, 0.f, MaxFloatValue);
				    GUI::Properties::CheckboxProperty(
				        &component.EnableCollision, "Enable collision",
				        "Whether connected by joint bodies should collide with each other");
				    GUI::Properties::CheckboxProperty(&component.AutoLength, "Auto length",
				                                      "Whether the distance should be automatically calculated");
				    if (!component.AutoLength)
				    {
					    GUI::Properties::ScalarDragProperty<f32>(&component.Length, "Length",
					                                             "The distance between the two bodies", 1.f, 0.f);
				    }
				    GUI::Properties::ScalarDragProperty<f32>(&component.MinLength, "Min Length",
				                                             "The minimum distance between the two bodies", 1.f, 0.f,
				                                             component.MaxLength);
				    GUI::Properties::ScalarDragProperty<f32>(&component.MaxLength, "Max Length",
				                                             "The maximum distance between the two bodies", 1.f,
				                                             component.MinLength);
				    GUI::Properties::EndProperties();
			    },
			    true);

			DrawComponent<RevolutionJoint2DComponent>(
			    entity,
			    [this](RevolutionJoint2DComponent& component) {
				    ...
				    GUI::Properties::CheckboxProperty(
				        &component.EnableCollision, "Enable collision",
				        "Whether connected by joint bodies should collide with each other");
				    GUI::Properties::CheckboxProperty(&component.EnableLimit, "Limit Enabled",
				                                      "Whether the joint should have a limit");
				    if (component.EnableLimit)
				    {
					    f32 lowerAngle = glm::degrees(component.LowerAngle);
					    GUI::Properties::ScalarDragProperty<f32>(&lowerAngle, "Lower Angle",
					                                             "The lower angle for the joint limit (in degrees)");
					    component.LowerAngle = glm::radians(lowerAngle);

					    f32 upperAngle = glm::degrees(component.UpperAngle);
					    GUI::Properties::ScalarDragProperty<f32>(&upperAngle, "Upper Angle",
					                                             "The upper angle for the joint limit (in degrees)");
					    component.UpperAngle = glm::radians(upperAngle);
				    }
				    GUI::Properties::CheckboxProperty(&component.EnableMotor, "Enable Motor",
				                                      "Whether the joint should have a motor");
				    if (component.EnableMotor)
				    {
					    GUI::Properties::ScalarDragProperty<f32>(&component.MotorSpeed, "Motor Speed",
					                                             "The speed of the motor in radians per second");
					    GUI::Properties::ScalarDragProperty<f32>(&component.MaxMotorTorque, "Max Motor Torque",
					                                             "The maximum torque of the motor in Newtons");
				    }
				    GUI::Properties::ScalarDragProperty<f32>(&component.BreakingTorque, "Breaking Torque",
				                                             "The torque acting on the joint to break it in Newtons",
				                                             1.f, 0.f);
				    GUI::Properties::ScalarDragProperty<f32>(&component.BreakingForce, "Breaking Force",
				                                             "The force acting on the joint to break it in Newtons",
				                                             1.f, 0.f);
			...
			DrawComponent<PrismaticJoint2DComponent>(
			    entity,
			    [this](PrismaticJoint2DComponent& component) {
				    ...
				    if (component.EnableLimit)
				    {
					    ...
					    GUI::Properties::ScalarDragProperty<f32>(
					        &angle, "Angle", "The constrained angle between the bodies (in degrees)");
				    ...
				    {
					    GUI::Properties::ScalarDragProperty<f32>(&component.MotorSpeed, "Motor Speed",
					                                             "The speed of the motor in radians per second", 1.f,
					                                             0.f);
					    GUI::Properties::ScalarDragProperty<f32>(&component.MaxMotorForce, "Max Motor Force",
					                                             "The maximum force of the motor in Newtons", 1.f, 0.f);
				    }
				    GUI::Properties::EndProperties();
			    },
			    true);

			DrawComponent<SpringJoint2DComponent>(
			    entity,
			    [this](SpringJoint2DComponent& component) {
				    ...
				    GUI::Properties::ScalarDragProperty<f32>(&component.MaxLength, "Max Length",
				                                             "The maximum distance between the two bodies", 1.f,
				                                             component.MinLength, MaxFloatValue);
				    GUI::Properties::ScalarDragProperty<f32>(&component.Frequency, "Frequency",
				                                             "The linear stiffness in N/m.");
				    GUI::Properties::ScalarSliderProperty(&component.DampingRatio, "Damping Ratio",
				                                          "The linear damping in N*s/m.", 0.0f, 1.0f);
				    GUI::Properties::EndProperties();
			    },
			    true);

			DrawComponent<WheelJoint2DComponent>(
			    entity,
			    [this](WheelJoint2DComponent& component) {
				    ...
				    GUI::Properties::ScalarDragProperty<f32>(&component.Frequency, "Frequency",
				                                             "The linear stiffness in N/m.");
				    GUI::Properties::ScalarDragProperty<f32>(&component.DampingRatio, "Damping Ratio",
				                                             "The linear damping in N*s/m.", 1.f, 0.0f, 1.0f);
				    GUI::Properties::EndProperties();
			    },
			    true);
			...
			DrawComponent<AudioSourceComponent>(
			    ...
			    [&](AudioSourceComponent& component) {
				    ...
				    GUI::Properties::AssetSearchProperty<Sound>(&component.Handle, "Clip",
				                                                "The audio sound to be played");
				    GUI::Properties::ScalarSliderProperty<f32>(&component.Volume, "Volume",
				                                               "The volume of the audio source", 0.f, 2.f);
				    GUI::Properties::ScalarSliderProperty<f32>(&component.Pitch, "Pitch",
				                                               "The pitch of the audio source", 0.f, 2.f);
				    GUI::Properties::CheckboxProperty(&component.Looping, "Looping",
				                                      "Whether the audio source should loop");
				    GUI::Properties::CheckboxProperty(&component.PlayOnCreate, "Play On Create",
				                                      "Whether the audio source should play on entity creation");
				    GUI::Properties::CheckboxProperty(&component.Is3D, "3D", "Whether the audio source should be 3D");

				    if (component.Is3D)
				    {
					    GUI::Properties::SelectableProperty<AttenuationType>(
					    ...
					    GUI::Properties::ScalarSliderProperty<f32>(
					        &component.RollOff, "Roll Off",
					        "The roll off factor of the audio source (how quickly the sound fades)", 0.f, 10.f);
					    GUI::Properties::ScalarSliderProperty<f32>(&component.MinGain, "Min Gain",
					                                               "The minimum gain of the audio source", 0.f, 1.f);
					    GUI::Properties::ScalarSliderProperty<f32>(&component.MaxGain, "Max Gain",
					                                               "The maximum gain of the audio source", 0.f, 1.f);
					    GUI::Properties::ScalarSliderProperty<f32>(
					        &component.MinDistance, "Min Distance",
					        "The minimum distance of the audio source (where the sound starts to fade)", 0.f, 1000.f);
					    GUI::Properties::ScalarSliderProperty<f32>(
					        &component.MaxDistance, "Max Distance",
					        "The maximum distance of the audio source (where the sound is inaudible)", 0.f, 1000.f);

					    // f32 coneInnerAngle = glm::degrees(component.ConeInnerAngle);
					    // if (GUI::Properties::ScalarDragProperty<f32>(&coneInnerAngle, "Cone Inner Angle", "The inner
					    // angle of the cone in degrees", 1.f, 0.f, 360.f)) { component.ConeInnerAngle =
					    // glm::radians(coneInnerAngle);
					    //}

					    // f32 coneOuterAngle = glm::degrees(component.ConeOuterAngle);
					    // if (GUI::Properties::ScalarDragProperty<f32>(&coneOuterAngle, "Cone Outer Angle", "The outer
					    // angle of the cone in degrees", 1.f, 0.f, 360.f)) { component.ConeOuterAngle =
					    // glm::radians(coneOuterAngle);
					    //}

					    // GUI::Properties::ScalarSliderProperty<f32>(&component.ConeOuterGain, "Cone Outer Gain", "The
					    // gain of the outer cone", 0.f, 1.f);
					    // GUI::Properties::ScalarSliderProperty<f32>(&component.DopplerFactor, "Doppler Factor", "The
					    // Doppler factor of the audio source", 0.f, 10.f);
				    }
			...
			DrawComponent<AudioListenerComponent>(
			    ...
			    [](AudioListenerComponent& /*component*/) {
				    ...
				    // GUI::Properties::ScalarSliderProperty<f32>(&component.ConeOuterGain, "Cone Outer Gain", "The gain
				    // of the outer cone", 0.f, 1.f);
