...
namespace SW
{
	...
	StatisticsPanel::StatisticsPanel() : Panel("Statistics", SW_ICON_INFORMATION_VARIANT, true)
	{
	}
	...
	void StatisticsPanel::OnRender()
	{
		...
		if (OnBegin())
		{
			...
			{
				const f32 frameTime = m_FrameTimes[i];
			...
			ImGui::PlotLines("##FPS", m_FpsValues, static_cast<int>(size), 0, nullptr, 0,
			                 m_FpsValues[0] + m_FpsValues[0] / 2.f, graphSize);

			static constexpr ImGuiTreeNodeFlags treeFlags =
			    ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_SpanAvailWidth |
			    ImGuiTreeNodeFlags_AllowItemOverlap | ImGuiTreeNodeFlags_Framed | ImGuiTreeNodeFlags_Bullet;
			...
			if (ImGui::TreeNodeEx("##stats_tree_node", treeFlags, "%s", SW_ICON_CHART_BAR "  Statistics"))
			{
				...
				GUI::Properties::SingleLineTextInputProperty(&frameTime, "Frame time (ms)", nullptr,
				                                             ImGuiInputTextFlags_ReadOnly);

				std::string drawCalls = std::to_string(Renderer2D::GetStats().DrawCalls);
				GUI::Properties::SingleLineTextInputProperty(&drawCalls, "2D Draw Calls", nullptr,
				                                             ImGuiInputTextFlags_ReadOnly);

				std::string quadCount = std::to_string(Renderer2D::GetStats().QuadCount);
				GUI::Properties::SingleLineTextInputProperty(&quadCount, "Quads", nullptr,
				                                             ImGuiInputTextFlags_ReadOnly);

				std::string vertexCount = std::to_string(Renderer2D::GetStats().GetTotalVertexCount());
				GUI::Properties::SingleLineTextInputProperty(&vertexCount, "Vertices", nullptr,
				                                             ImGuiInputTextFlags_ReadOnly);

				std::string indexCount = std::to_string(Renderer2D::GetStats().GetTotalIndexCount());
				GUI::Properties::SingleLineTextInputProperty(&indexCount, "Indices", nullptr,
				                                             ImGuiInputTextFlags_ReadOnly);
			...
			if (ImGui::TreeNodeEx("##settings_tree_node", treeFlags, "%s",
			                      SW_ICON_SETTINGS_OUTLINE "  Editor Settings"))
			{
