...
namespace SW
{
	AudioEventsPanel::AudioEventsPanel() : Panel("Audio Events", SW_ICON_VOLUME_HIGH, false)
	{
	}
	...
	void AudioEventsPanel::OnRender()
	{
		if (OnBegin())
		{
			...
			if (ImGui::BeginTable("AudioEventsPanel_MainViewTable", 2, flags | ImGuiTableFlags_ScrollY,
			                      ImGui::GetContentRegionAvail()))
			{
				...
				else
				{
					...
					GUI::Properties::AssetSearchProperty<Sound>(&m_SelectedEvent->Handle, "Clip",
					                                            "The audio sound to be played");
					GUI::Properties::SelectableProperty<AudioEventTriggerType>(
					    &m_SelectedEvent->TriggerType, {{"On Script Trigger", AudioEventTriggerType::OnScriptTrigger}},
					    "Trigger Type", "The type of trigger for the audio event");
					GUI::Properties::ScalarSliderProperty<f32>(&m_SelectedEvent->Volume, "Volume",
					                                           "The volume of the audio source", 0.f, 2.f);
					GUI::Properties::ScalarSliderProperty<f32>(&m_SelectedEvent->Pitch, "Pitch",
					                                           "The pitch of the audio source", 0.f, 2.f);
					GUI::Properties::CheckboxProperty(&m_SelectedEvent->Looping, "Looping",
