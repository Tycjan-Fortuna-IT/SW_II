...
namespace SW
{
	...
	SpritesheetEditor::SpritesheetEditor(const char* name, const char* icon) : AssetEditorPanel(name, icon)
	{
	}
	...
	void SpritesheetEditor::Render()
	{
		...
		ImGui::Text("Currently editing: %s [%i x %i]", m_Name.c_str(),
		...
		if (ImGui::BeginTable("MainViewTable", 2, flags, ImGui::GetContentRegionAvail()))
		{
			...
			if (GUI::Layout::BeginHeaderCollapse("General Options", ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				GUI::Properties::ScalarSliderProperty(&(*m_Spritesheet)->GridSize, "Grid Size", nullptr, MIN_GRID_SIZE,
				                                      MAX_GRID_SIZE);
				GUI::Properties::Vector2InputProperty(&(*m_Spritesheet)->CenterOffset, "Center Offset");
				GUI::Properties::DrawFolderPickerProperty(&(*m_Spritesheet)->ExportPath,
			...
			if (GUI::Layout::BeginHeaderCollapse("Advanced Options"))
			{
				...
				if (GUI::Properties::ScalarSliderProperty(&(*m_Spritesheet)->ViewZoom, "View Zoom", nullptr, MIN_ZOOM,
				                                          MAX_ZOOM))
				{
					m_Canvas.SetView(viewOrigin, (*m_Spritesheet)->ViewZoom);
				}
			...
			if (m_Canvas.Begin("##mycanvas", ImGui::GetContentRegionAvail()))
			{
				...
				else if (!isDragging && ImGui::IsItemHovered() && io.MouseWheel)
				{
					...
					const f32 newViewScale =
					    glm::clamp(m_Canvas.ViewScale() * powf(1.1f, io.MouseWheel), MIN_ZOOM, MAX_ZOOM);
				...
				for (f32 x = fmodf(loffset.x, gridScale); x < viewSize.x; x += gridScale)
				{
					...
					drawList->AddLine(ImVec2(x - offset.x - 0.5f, 0.0f - offset.y) + viewPos,
					                  ImVec2(x - offset.x - 0.5f, viewSize.y - offset.y) + viewPos,
					                  GUI::Theme::Selection, 2.f);
					...
					if ((*m_Spritesheet)->ShowGridCoords)
					{
						for (f32 y = fmodf(loffset.y, gridScale); y < viewSize.y; y += gridScale)
						{
							...
							drawList->AddText(ImVec2(x - offset.x - 0.5f, y - offset.y - 0.5f) + viewPos,
				...
				for (f32 y = fmodf(loffset.y, gridScale); y < viewSize.y; y += gridScale)
				{
					...
					drawList->AddLine(ImVec2(0.0f - offset.x, y - offset.y - 0.5f) + viewPos,
					                  ImVec2(viewSize.x - offset.x, y - offset.y - 0.5f) + viewPos,
					                  GUI::Theme::Selection, 2.f);
				}
	...
	void SpritesheetEditor::DrawRectOnCanvas(ImDrawList* drawList, const glm::vec2& position, const glm::vec2& size,
	                                         u32 color) const
	{
		...
		if (ImGui::BeginTable("SideViewTable", 1, flags, ImGui::GetContentRegionAvail()))
		{
			...
			for (int i = 0; i < (int)sprites.size(); i++)
			{
				...
				if (opened)
				{
					...
					GUI::Properties::Vector2InputProperty(&sprite.Position, "Position", nullptr, 0.f, 0.f, FLT_MAX,
					                                      "%1.f");
					GUI::Properties::Vector2InputProperty(&sprite.Size, "Size", nullptr, (*m_Spritesheet)->GridSize,
					                                      1.f, 1024.f, "%1.f");
					glm::vec2 position = sprite.Position + (*m_Spritesheet)->CenterOffset;
					GUI::Properties::DrawImagePartProperty((*m_Spritesheet)->GetSpritesheetTexture(), "Sprite", nullptr,
					                                       position, sprite.Size, glm::vec4(1.0f),
	...
	void SpritesheetEditor::ExportSprites() const
	{
		...
		for (const SpriteData& sprite : (*m_Spritesheet)->Sprites)
		{
			...
			std::filesystem::path path = ProjectContext::Get()->GetAssetDirectory() / (*m_Spritesheet)->ExportPath /
			                             (sprite.Name + ".sw_sprite");
	...
	void SpritesheetEditor::GenerateFromGridPopup(bool* opened)
	{
		...
		if (ImGui::BeginPopupModal(SW_ICON_GRID " Generate sprites from a grid", nullptr))
		{
			...
			GUI::Properties::Vector2InputProperty(
			    &gridSize, "Grid Size",
			    "Grid size used to generate sprites, can be different from the current grid size.", currentGridSize,
			    MIN_GRID_SIZE, MAX_GRID_SIZE, "%1.f");

			static glm::vec2 textureOffset = {0.f, 0.f};
			GUI::Properties::Vector2InputProperty(
			    &textureOffset, "Texture Offset",
			    "Offset in texture, used to start cutting sprites from a different position.", 0.f, 0.f, FLT_MAX,
			    "%1.f");

			static f32 cutSizeX = (f32)texture->GetWidth();
			GUI::Properties::ScalarInputProperty(&cutSizeX, "Cut Size X",
			                                     "Cut size in X axis, part or whole spritesheet texture.", 16.f, 32.f,
			                                     0.f, (f32)texture->GetWidth(), "%1.f");

			static f32 cutSizeY = (f32)texture->GetHeight();
			GUI::Properties::ScalarInputProperty(&cutSizeY, "Cut Size Y",
			                                     "Cut size in Y axis, part or whole spritesheet texture.", 16.f, 32.f,
			                                     0.f, (f32)texture->GetHeight(), "%1.f");
