...
namespace SW
{
	AnimationEditor::AnimationEditor(const char* name, const char* icon) : AssetEditorPanel(name, icon)
	{
	}
	...
	void AnimationEditor::Render()
	{
		...
		ImGui::Text("Currently editing: %s  %llu frames", m_Name.c_str(), (*m_Animation)->Sprites.size());
		ImGui::PopFont();
		...
		if (ImGui::BeginTable("AnimationEditor_MainViewTable", 2, flags, ImGui::GetContentRegionAvail()))
		{
			...
			if (m_FramesCount)
			{
				...
				const f32 sprWidth  = std::roundf(abs((*current)->TexCordUpRight.x - (*current)->TexCordLeftDown.x) *
				                                  (f32)texture->GetWidth());
				const f32 sprHeight = std::roundf(abs((*current)->TexCordUpRight.y - (*current)->TexCordLeftDown.y) *
